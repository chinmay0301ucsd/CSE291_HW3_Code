...
def test(params):
	...
	netC = Nerf().cuda()
	netF = Nerf().cuda()
	netC.load_state_dict(torch.load(params['loadpathC']),strict=True)
	netF.load_state_dict(torch.load(params['loadpathF']),strict=True)

	if params['animation']:
		theta = -params['theta']
		n_phi = params['num_poses']
		poses = poses_to_render(r=4, theta=theta, n_phi=n_phi)
		render_poses(netF, poses, cam_params, batch_size, savepath,\
