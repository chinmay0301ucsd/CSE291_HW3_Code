...
def rays_single_cam(cam_params, spp=1, zdir=1):
	...
	"""
	H , W, f  = cam_params
	Hl = torch.arange(H) - H//2 + 0.5
	Wl = torch.arange(W) - W//2 + 0.5
	...
	rays = torch.stack((grid_x/f, grid_y/f, zdir*torch.ones_like(grid_x))).float()
	# rays = torch.stack((grid_x/f, grid_y/f, zdir*torch.ones_like(grid_x))).float()
	...
	# rays = torch.stack((grid_y/f, grid_x/f, 1*torch.ones_like(grid_x))).float()
	# ## TODO REVERT LATER
	# # rays = rays.permute(0,2,1)
	# rays = torch.reshape(rays, (3,-1)) # 640K ray directions (if H,W = 800 and spp=1 else 640K*n)
	# return rays
